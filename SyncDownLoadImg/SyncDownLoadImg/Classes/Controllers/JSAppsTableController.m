...
@implementation JSAppsTableController{
    ...
    NSString                     *_currentIconString;       //  记录当前图片UrlString
    NSMutableDictionary          *_previouseIconString;     //  记录Cell的UrlString;
    NSMutableDictionary          *_imageCache;              //  图片缓存池 (抽取到下载操作管理类)
    NSMutableDictionary          *_operationCache;          //  操作缓存池 (抽取到下载操作管理类)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    // 取消之前图片的下载操作
    if ( ![model.icon isEqualToString:_currentIconString] ) {
        
        [[JSDownLoadOperationManager sharedManager] cancelOperationWithUrlString:_currentIconString];
    }
    _currentIconString = model.icon;
    
    // 设置占位图
    cell.imageView.image = [UIImage imageNamed:@"user_default"];
    
    // 使用下载操作管理类下载图片
    [[JSDownLoadOperationManager sharedManager] downloadImageWithUrlstring:model.icon withCompleteHandler:^(UIImage *image) {
